...
// Feature: RAG over evidence-based parenting literature (AAP, NICHD) with safe-content filters
...
export default function RagOverEvidenceBasedParentingLiteratureAapNPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-rag-over-evidence-based-parenting-literature-aap-n/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over evidence-based parenting literature (AAP, NICHD) with safe-content filters</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-evidence-based-parenting-literature-aap-n/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
