...
export default function Only4AiEndpointsNarrowCoveragePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-only-4-ai-endpoints-narrow-coverage/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 4 AI endpoints — narrow coverage</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-4-ai-endpoints-narrow-coverage/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
